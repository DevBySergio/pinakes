...
const legacyPinakeDirectoryName = 'Pinake';
...
async function validateMarkdownDocuments(root, issues) {
	...
	for (const relativePath of await collectMarkdownFiles(docsDirectory, [])) {
		...
		for (const target of extractMarkdownLinkTargets(content)) {
			...
			if (!candidates || await anyMarkdownLinkCandidateExists(docsDirectory, candidates)) {
				continue;
			}
...
async function anyMarkdownLinkCandidateExists(docsDirectory, candidates) {
	for (const candidate of candidates) {
		if (await exists(path.join(docsDirectory, candidate))) {
			return true;
		}
	}
...
function resolveMarkdownLinkCandidates(sourceRelativePath, rawTarget) {
...
function parseMarkdownLinkDestination(rawValue) {
...
function findMarkdownTitleStart(value) {
...
function isMarkdownLinkTitle(value) {
	if (value.length < 2) {
		return false;
	}
...
function shouldIgnoreMarkdownLinkTarget(target) {
